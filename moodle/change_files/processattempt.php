...
if ($status == quiz_attempt::OVERDUE) {
    ...
    // отправляю пост запрос на сервер с моим приложением
    $id = required_param('cmid', PARAM_INT); // Course module id
    $quizobj = quiz_settings::create_for_cmid($id, $USER->id);
    $quiz = $quizobj->get_quiz();
    $req = new Request($CFG->wwwproctorecg . "/close_check_ecg");
    $data = array('user_id' => $USER->id, 'quiz_id' => $quiz->id);
    $json = json_encode($data);
    $data_2 = array('json' => $json);
    print_r($json);
    print_r($req->httpPost($CFG->wwwproctorecg . "/close_check_ecg", $data_2));
