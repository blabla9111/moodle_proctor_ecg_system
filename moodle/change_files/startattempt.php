...
// Этот блок кода добавлен для контроля действий пользователя в proctor_ecg
$quiz = $quizobj->get_quiz();
$check_proctor_egc_course_flag = $DB->get_records_sql_menu('SELECT * FROM {proctor_ecg_course} WHERE quiz_id = ? and proctor_ecg_flag = 1', [$quiz->id]);
if (!empty($check_proctor_egc_course_flag)) {

    $check_proctor_start_flag = $DB->get_records_sql_menu('SELECT * FROM {proctor_user} WHERE user_id = ? AND quiz_id = ? AND proctor_flag = 0 AND proctor_start_flag = 1', [$USER->id, $quiz->id]);
    print_r($check_proctor_start_flag);
    if (empty($check_proctor_start_flag)) {
...
if ($accessmanager->is_preflight_check_required($currentattemptid)) {
    ...
    $mform = $accessmanager->get_preflight_check_form(
        $quizobj->start_attempt_url($page),
        $currentattemptid
    );
...
// $quiz = $quizobj->get_quiz();
// print_r($quiz->id);
// print_r($DB->get_records_sql_menu('SELECT TRUE FROM {proctor_user} WHERE user_id = ? AND quiz_id = ? AND proctor_flag = 0',[$USER->id, $quiz->id]));
